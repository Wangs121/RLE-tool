...
export const Editor: React.FC<EditorProps> = ({ title, code, readOnly, onChange, action }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 rounded-xl border border-slate-800 overflow-hidden shadow-xl">
      ...
      <textarea
        className={`flex-1 w-full bg-slate-900 p-4 font-mono text-sm leading-6 resize-none focus:outline-none focus:ring-0 ${readOnly ? 'text-slate-400' : 'text-slate-200'}`}
        value={code}
